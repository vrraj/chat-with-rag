...
While this repository includes example tools and sample datasets, it is designed as a
...
- **Indexing Pipeline** – Extracts content from documents, chunks text, embeds it, and stores structured vectors in Qdrant.
...
The indexing pipeline provides a **batch ingestion mode** driven by a JSON specification. A single batch file can describe a heterogeneous set of sources—local files and remote URLs—that are processed in one run.
...
** Estimation Mode **
The indexing pipeline supports an **estimation mode** that runs extraction and chunking steps without calling the embedding model or writing to the index. This mode is useful for:

- Quickly previewing how many chunks a document will produce
- Validating extraction rules or section skipping logic
- Estimating embedding costs before committing to full ingestion

Estimation mode can be triggered via CLI flags or configuration settings and outputs summary statistics and chunk previews to the console or logs.

### 3. High-Level Flow

At a high level, the indexing pipeline follows this sequence:
...
- The frontend provides a **View Documents** view that displays all indexed documents
...
The embedding flow transforms text chunks produced by the chunking stage into high‑dimensional vector embeddings suitable for semantic retrieval. This component is designed to be model‑agnostic, efficient, and fully metadata‑preserving.
...
- Switching embedding models without modifying pipeline logic
- Configuring model parameters in one place
- Adding retries, batching, and API‑related safeguards

This abstraction keeps the ingestion pipeline decoupled from model implementation details.
...
The orchestration layer supports LLM-initiated **tool calls** during the reasoning phase when tools are enabled.

Tools are used for operations such as:

- structured lookup against the vector store
- metadata or context inspection
- retrieving additional document snippets
- executing project-specific utilities
...
3. executes the tools through a registry of executors
4. aggregates tool outputs
5. performs an additional synthesis pass to merge the RAG draft answer and tool results into the final response

For observability, the pipeline emits a single `Tool Calls` SSE stage before running this loop. Individual tool executions are logged but are not streamed as separate `tool_started` / `tool_completed` events.
...
The retrieval and ranking subsystem identifies the most semantically relevant document chunks to support the LLM's answer generation. It operates in two phases: vector retrieval and heuristic/LLM-based reranking.

### 1. Query Embedding
A single embedding is generated for the rewritten user query. This embedding is used as the vector query against Qdrant.
...
- positional/section-weighted adjustments
...
### 6. Deduplication and Final Selection
Final chunks are deduplicated, sorted, and trimmed to produce a coherent, size‑bounded context window for inference.
...
The system tracks detailed metrics during ingestion and chat execution.

### 1. Embedding Metrics
- token usage per chunk and per document
- total embedding cost estimation
- retry counts
- failure patterns
...
This clarifies that log files are automatically rotated and will not grow indefinitely during long-running use.

## Configuration and Settings

The system is highly configurable through environment variables and a centralized settings class.

### 1. Model Configuration
- embedding model
- inference model
- rerank model
- tools synthesis model
...
- **Frontend Integration:** embedding the chat UI into an existing employee portal or
  customer‑facing application, or wiring the backend into a different user interface.

This architecture emphasizes modularity and composability so that organizations can
adopt the system as a foundation and evolve it according to their domain‑specific
requirements.
...
## 9. Developer & Operator Utilities (Makefile) 🧑‍💻
