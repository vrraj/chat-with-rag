...
try:
    ...
    try:
        from llm.modelspec import ModelSpec
    except Exception:
        from llm.ModelSpec import ModelSpec
...
class LLMHandler:
    ...
    def _gemini_embedding_call(self, *, model: str, input: Any, **kwargs: Any):
        ...
        The typical model is `gemini-embedding-001`. If `dimensions` is not
        provided, we default to 1536, which is one of the supported sizes
        (3072, 1536, 768).
        """
        client = self._get_gemini()
        # Provide a sensible default for dimensions if the caller does not specify one.
