function ChatSection() {
        ...
        try {
            ...
            const response = await fetch(`/chat/${session}`, {
                ...
                body: JSON.stringify({
                    ...
                    params: { use_tools: useToolsEnabled }
                }),
            });
    ...
    const handleUseToolsToggle = () => {
        setUseToolsEnabled(!useToolsEnabled);
    };
    ...
    return (
        <div className="space-y-4">
                    ...
                    <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'} mb-4`}>
                        <div className={`max-w-[80%] p-3 rounded-lg ${
                            ...
                            {message.tools_used && message.tools_used.length > 0 && (
                                <div className="mt-2 text-xs text-gray-500">Tools Used: {message.tools_used.join(', ')}</div>
                            )}
            ...
            <label className="relative inline-flex items-center cursor-pointer mb-4 ml-4">
                <input 
                    type="checkbox" 
                    checked={useToolsEnabled}
                    onChange={handleUseToolsToggle}
                    className="sr-only peer"
                />
                <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                <span className="ml-3 text-sm font-medium text-gray-900">Use Tools</span>
            </label>
