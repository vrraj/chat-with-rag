...
def _load_yaml(path: str) -> Dict[str, Any]:
    ...
    if not path or not os.path.exists(path):
        raise LLMError(
            provider="internal",
            kind="config",
            message=f"Inference prompt registry YAML file not found at path: {path}",
        )
    ...
    try:
        ...
        raise LLMError(
            provider="internal",
            kind="config",
            message=f"Failed to read/parse inference prompt registry YAML at {path}: {e}",
        )
    ...
    if not isinstance(data, dict):
        raise LLMError(
            provider="internal",
            kind="config",
            message=f"Inference prompt registry YAML must be a mapping/object at top-level: {path}",
        )
