...
- **OpenAI**
  - Text: via `llm_handler.responses.create(...)`.
  - Embeddings: via `llm_handler.embeddings.create(...)`.
- **Gemini (via OpenAI-compatible adapter)**
  - Text: primarily via `llm_handler.create(...)` with provider="gemini"
  - Embeddings: via `llm_handler.embeddings.create(...)` if configured.
