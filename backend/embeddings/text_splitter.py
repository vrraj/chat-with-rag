...
class TextSplitter:
    ...
    def split_text(self, text: str) -> List[str]:
        if self.use_manual_splitter:
            ...
            print(f"[DEBUG] Manual mode token count: {len(tokens)}")
            chunks = []
            i = 0
            while i < len(tokens):
            ...
            print(f"[DEBUG] Total chunks created (manual): {len(chunks)}")
        else:
            # Use Langchain's TokenTextSplitter for splitting
            chunks = self.splitter.split_text(text)
            enc = tiktoken.get_encoding("cl100k_base")
            token_count = len(enc.encode(text))
            print(f"[DEBUG] Langchain mode token count: {token_count}")
            print(f"[DEBUG] Total chunks created (langchain): {len(chunks)}")
